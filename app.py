from click import option
import streamlit as stl
import pickle
import pandas as pd
import requests

def fetch_poster(movie_id):
    
    
    url=('https://api.themoviedb.org/3/movie/{}?api_key=2f5caaa42c0b7ad87cbc40d500981d3d&language=en-US'.format(movie_id))
    data = requests.get(url)
    data = data.json()
    poster_path = data['poster_path']
    full_path = "https://image.tmdb.org/t/p/w500/" + poster_path
    return full_path
    
    
def recommend(movie):
    movie_index=movies[movies['title']==movie].index[0]
    distances=similiarity[movie_index]
    movie_list=sorted(list(enumerate(distances)),reverse=True,key=lambda x:x[1])[1:6]
    recommended_movies=[]
    recommended_movies_posters=[]
    
    for i in movie_list:
        movie_id= movies.iloc[i[0]].id
        
        recommended_movies.append(movies.iloc[i[0]].title)
        # fetch Poster from api
        recommended_movies_posters.append(fetch_poster(movie_id))
    return recommended_movies,recommended_movies_posters

movies_dict = pickle.load(open('movie_dict1.pkl','rb'))

movies = pd.DataFrame(movies_dict)
similiarity= pickle.load(open('similiar.pkl','rb'))

stl.title("Movie Recommender System")
selected_movie_name= stl.selectbox(
'Type Or Select The Movie Name:-',
movies['title'].values)

if stl.button('Recommend'):
    
    recommended_movies,recommended_movies_posters = recommend(selected_movie_name)
    
    col1,col2,col3,col4,col5= stl.columns(5)
    with col1:
        stl.text(recommended_movies[0])
        stl.image(recommended_movies_posters[0])
        
    with col2:
        stl.text(recommended_movies[1])
        stl.image(recommended_movies_posters[1])
   
    with col3:
        stl.text(recommended_movies[2])
        stl.image(recommended_movies_posters[2])
   
    with col4:
        stl.text(recommended_movies[3])
        stl.image(recommended_movies_posters[3])
   
    with col5:
        stl.text(recommended_movies[4])
        stl.image(recommended_movies_posters[4])
   
   
